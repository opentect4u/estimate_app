...
async def categorywise_item_list(catg:SearchByCategory):
    ...
    query = f"SELECT a.*, b.*, c.unit_name FROM md_items a JOIN md_item_rate b on a.id=b.item_id LEFT JOIN md_unit c on c.sl_no=a.unit_id WHERE a.comp_id={catg.comp_id} AND a.catg_id={catg.catg_id} " if catg.catg_id>0 else f"SELECT a.*, b.*, c.unit_name FROM md_items a JOIN md_item_rate b on a.id=b.item_id LEFT JOIN md_unit c on c.sl_no=a.unit_id WHERE a.comp_id={catg.comp_id}"
    cursor.execute(query)
    records = cursor.fetchall()
    print(query)
    result = createResponse(records, cursor.column_names, 1)
