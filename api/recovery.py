                ...
                if cursor.rowcount>0:
                    try:
                        if recover.payment_mode == 'U':
                            ...
                            query = f"INSERT INTO td_txn_dtls (receipt_no,pay_txn_id,pay_amount,pay_amount_original,currency_code,payment_mode,pay_status,receipt_url,created_by,created_dt) VALUES ('{recover.customer_mobile}','{recover.pay_txn_id}',{recover.pay_amount},{recover.pay_amount_original},'{recover.currency_code}','{recover.payment_mode}','{recover.pay_status}','{recover.receipt_url}','{recover.user_id}','{formatted_dt}')"
                            ...
                            cursor.close()
                            if cursor.rowcount>0:
                                resData = {"status":1, "msg":"All Data And Transaction Details Inserted Successfully"}
                            else:
                                resData = {"status":1, "msg":"Failed to Insert Transaction Details"}
                        else:
                            resData = {"status":1, "msg":"All Data Inserted Successfully"} 
                    except:
                        print('+++++++ Error in Tnsx ++++++++++')
                else:
                    resData = {"status":0, "data":"Data not inserted in td_recovery"}
