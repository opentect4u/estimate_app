...
async def userwise_report(data:UserwiseReport):
    ...
    query = f"Select c.user_name user_name,a.created_by user_id, sum(a.net_amt) net_amt, count(a.receipt_no) receipt_no_count from td_receipt a, md_user c,td_item_sale b where a.receipt_no = b.receipt_no and a.created_by=c.user_id and a.trn_date BETWEEN '{data.from_date}' and '{data.to_date}' and b.cancel_flag = 0 and a.comp_id = {data.comp_id} AND a.br_id = {data.br_id} and user_id='{data.user_id}'  group by c.user_name,a.created_by"
...
async def Productwise_report(item_rep:ItemReport):
    ...
    query = f"SELECT a.trn_date, b.item_name,a.item_id,d.unit_name as unit_name,e.category_name,SUM(a.qty)as tot_item_qty,c.price as unit_price,sum(a.price*a.qty)as tot_item_price from td_item_sale a, md_items b, md_item_rate c,md_unit d,md_category e,md_brand f where a.item_id = b.id and a.item_id=c.item_id  and b.unit_id = d.sl_no and a.comp_id = {item_rep.comp_id} and a.br_id = {item_rep.br_id} and a.trn_date >= '{item_rep.from_date}' and a.trn_date <= '{item_rep.to_date}' and b.catg_id=e.sl_no and a.cancel_flag = 0 group by b.item_name,a.item_id,d.unit_name,c.price"
