...
async def register(rcpt:list[Receipt]):
    ...
    receipt=f'{rcpt[0].user_name[0:3]}{rcpt[0].branch_name[0:3]}{receipt_time}'
    for i in rcpt:
...
# ============================================================================================================
# Estimate App Transactions

@tnxRouter.get('/receipt_url')
async def receipt_url(receipt_no:str):
    conn = connect()
    cursor = conn.cursor()

    query = f"SELECT receipt_no, receipt_url FROM td_txn_dtls WHERE receipt_no={receipt_no}"
...
async def add_category(data:AddEditTXN):
    current_datetime = datetime.now()
    formatted_dt = current_datetime.strftime("%Y-%m-%d %H:%M:%S")
    conn = connect()
    cursor = conn.cursor()
    # sl_no = f""
    query = f"INSERT INTO td_txn_dtls(receipt_no, pay_txn_id, pay_amount, pay_amount_original, currency_code, payment_mode, pay_status, receipt_url, created_by, created_dt) VALUES ('{data.receipt_no}', '{data.pay_txn_id}', {data.pay_amount}, {data.pay_amount_original}, '{data.currency_code}', '{data.payment_mode}', '{data.pay_status}', '{data.receipt_url}', '{data.created_by}', '{formatted_dt}')"
    cursor.execute(query)
    conn.commit()
    conn.close()
    cursor.close()
    if cursor.rowcount>0:
        ...
        resData={
            "status":0,
            "data":"Failed to Add Transaction Details"
        }
    return resData