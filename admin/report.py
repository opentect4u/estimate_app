...
async def Productwise_report(item_rep:ItemReport):
    ...
    query = f"SELECT r.created_by as user_id,u.user_name,b.branch_name from md_user u,td_receipt r, count(r.receipt_no) as receipt_no_count,md_branch b where u.phone_no=r.created_by and b.id = r.br_id and r.comp_id = {item_rep.comp_id} and r.trn_date BETWEEN '{item_rep.from_date}' and '{item_rep.to_date}' group by r.created_by,u.user_name,b.branch_name,r.receipt_no" 
