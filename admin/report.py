...
async def Productwise_report(item_rep:ItemReport):
    ...
    query = f"SELECT r.created_by as user_id,count(r.receipt_no) as receipt_no_count,count(s.item_id) as item_count,u.user_name,b.branch_name from md_user u,td_receipt r,md_branch b,td_item_sale s where u.phone_no=r.created_by and b.id = r.br_id and s.receipt_no=r.receipt_no and r.comp_id = {item_rep.comp_id} and r.trn_date BETWEEN '{item_rep.from_date}' and '{item_rep.to_date}' group by r.created_by,u.user_name,b.branch_name" 
