...
async def Productwise_report(item_rep:ItemReport):
    ...
    query = f"SELECT r.created_by as user_id,count(r.receipt_no) as receipt_no_count,sum(s.qty) as item_count,sum(r.round_off) as tot_round_off,sum(r.net_amt) as net_sale,sum(r.net_amt)+sum(r.round_off) as gross_sale,
    SUM(CASE When r.pay_mode='C' Then r.net_amount Else 0 End ) as tot_cash_amt,
    SUM(CASE When r.pay_mode='R' Then CAmount Else 0 End ) as tot_credit_amt,
    SUM(CASE When r.pay_mode='U' Then CAmount Else 0 End ) as tot_UPI_amt,
    u.user_name,b.branch_name from md_user u,td_receipt r,md_branch b,td_item_sale s where u.phone_no=r.created_by and b.id = r.br_id and s.receipt_no=r.receipt_no and r.comp_id = {item_rep.comp_id} and s.trn_date BETWEEN '{item_rep.from_date}' and '{item_rep.to_date}'  group by r.created_by,u.user_name,b.branch_name" 
